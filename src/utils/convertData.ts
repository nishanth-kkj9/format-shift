...
function parseCsv(text: string, delimiter: string): string[][] {
  ...
  const endField = () => { fields.push(current.trim()); current = ''; };
  const endRecord = () => { endField(); rows.push(fields); fields = []; };
  while (i < src.length) {
    const char = src[i];
    if (inQuotes) {
      if (char === '"') {
...
export function jsonToXml(obj: unknown, rootName = 'root'): string {
  ...
  function buildXml(data: unknown, indent = '  ') {
    if (Array.isArray(data)) {
      data.forEach((item) => { xml += `${indent}<item>\n`; buildXml(item, indent + '  '); xml += `${indent}</item>\n`; });
    } else if (typeof data === 'object' && data !== null) {
      Object.entries(data as Record<string, unknown>).forEach(([key, val]) => {
        const cleanKey = key.replace(/[^a-zA-Z0-9_]/g, '_');
        if (typeof val === 'object' && val !== null) {
          xml += `${indent}<${cleanKey}>\n`; buildXml(val, indent + '  '); xml += `${indent}</${cleanKey}>\n`;
        } else { xml += `${indent}<${cleanKey}>${escapeXml(val)}</${cleanKey}>\n`; }
      });
    } else { xml += `${indent}${escapeXml(data)}\n`; }
  }
  buildXml(obj);
  xml += `</${rootName}>`;
  return xml;
...
function yamlScalar(value: unknown): string {
  if (typeof value === 'string') {
    if (value === '' || value === 'null' || value === 'true' || value === 'false' || value === 'yes' || value === 'no' || value === 'on' || value === 'off' || value === '~' || /^[-+0-9.]/.test(value) || /[:#&*!|>'"%@`]/.test(value) || /^\s|\s$/.test(value) || /[\n\r\t]/.test(value)) {
...
export function jsonToYaml(obj: unknown, indent = 0): string {
  ...
  if (Array.isArray(obj)) {
    obj.forEach((item) => {
      if (typeof item === 'object' && item !== null) { yaml += `${spaces}-\n${jsonToYaml(item, indent + 2)}`; }
      else { yaml += `${spaces}- ${yamlScalar(item)}\n`; }
    });
  } else if (typeof obj === 'object' && obj !== null) {
    Object.entries(obj as Record<string, unknown>).forEach(([key, val]) => {
      const safeKey = /[:#]/.test(key) ? JSON.stringify(key) : key;
      if (typeof val === 'object' && val !== null) { yaml += `${spaces}${safeKey}:\n${jsonToYaml(val, indent + 2)}`; }
      else { yaml += `${spaces}${safeKey}: ${yamlScalar(val)}\n`; }
    });
  } else { yaml += `${spaces}${yamlScalar(obj)}\n`; }
  return yaml;
}
...
function sanitizeUrl(url: string): string {
  ...
  try {
    ...
    if (ALLOWED_URL_SCHEMES.includes(parsed.protocol)) { return trimmed; }
